(function () {
    ...
    angular.module("sy.Ctgr.controller")
        .controller("sy.CtgrMenuCtrl", ["$scope", "sy.CtgrSvc", "sy.CodeSvc", "APP_CODE",
            function ($scope, SyAuthSvc, SyCodeSvc, APP_CODE) {
                ...
                menuVO.initLoad = function () {
                    ...
                    $scope.$on("authMng.menu:inquiry", function ($event, oSelect) {
                        if (!oSelect || !oSelect.NO_Ctgr) {
                            return;    
                        }
                        
                        if (self.treeApi.showData && self.treeApi.showData.length > 0) {
                            _datas[self.select.NO_Ctgr] = self.treeApi.showData.map(function(menuRow) {
                                return angular.copy(menuRow.entity);
                            });
                        }
                        
                        self.select.NO_Ctgr = oSelect.NO_Ctgr;
                        self.select.NM_Ctgr = oSelect.NM_Ctgr;
                        self.boxTitle = "메뉴정보 - "+ self.select.NM_Ctgr +" 선택 중";
                 ...
                 * @returns {{NO_Ctgr: string}}
                 */
                menuVO.makeReqParam = function () {
                    if (this.select.NO_Ctgr !== "") {
                        return {
                            NO_Ctgr : this.select.NO_Ctgr
                        };
                    }
                    return null;
                };
                ...
                menuVO.makeSaveParam  = function (postData) {
                    ...
                    function isDelete(datas, key) {
                        ...
                        var filterList = datas.filter(function(data) {
                            return (data.NO_Ctgr === key) && (data.STATE === "D"); 
                        });
                ...
                menuVO.inquiry = function () {
                    ...
                    SyAuthSvc.getMenuAuthList(self.makeReqParam()).then(function (result) {
                        _datas[self.select.NO_Ctgr] = self.options.data = result.data;
                    });
                };
